...
def safe_folder_name(value: str) -> str:
    """
    Convierte un identificador de paciente en un nombre seguro para carpeta.
    """
    return "".join(
...
def main():
    ...
    xml_files = sorted(INPUT_ROOT.glob("*.xml"))
    ...
    for p_idx, p_data in enumerate(patients_to_process, start=1):
        ...
        patient_folder_name = safe_folder_name(
        ...
        try:
            ...
            parser = TrialCriteriaParser(
                inclusion_llm=llm_client,
                exclusion_llm=llm_client,
                hardness_llm=llm_client,
            )
        ...
        # PredictionExporter usa ranking_result["patient_id"] como topic_id.
        # Por eso aquí guardamos el ID original del topic.
