...
class ClinicalMatchMaster:
    def __init__(
        ...
        """
        Args:
            generate_dossiers: Si True, genera los dossiers PDF finales por paciente.
            trec_year: None -> modo normal/live | 2021/2022 -> modo TREC
            max_trials: Número máximo de ensayos a procesar por paciente
        """
    ...
    def _normalize_trec_year(self, trec_year: int | None) -> int | None:
    ...
    def build_clinical_trials_client(self) -> ClinicalTrialsClient:
        ...
        if self.trec_year is None:
            ...
            print(f" Inicializando ClinicalTrialsClient en modo TREC {self.trec_year}...")
            self.clinical_trials_client = ClinicalTrialsClient(
                mode="trec",
    ...
    def generate_patient_dossier(self, ranking_output: dict[str, Any], output_path: str | Path) -> Path | None:
        output_path = Path(output_path)
        try:
            from dossier_generator import DossierGenerator
            DossierGenerator().generate_pdf(ranking_output=ranking_output, output_path=output_path)
    ...
    def run(self) -> None:
        ...
        for folder in [batch_folder, batch_output_folder, m16_inputs_folder, input_archived]:
            folder.mkdir(parents=True, exist_ok=True)
        ...
        try:
            input_adapter = InputAdapter(output_path=batch_folder / "adapted_patients.json")
        ...
        for p_idx, p_data in enumerate(patients_to_process, start=1):
            ...
            source_patient_id = str(p_data.get("source_patient_id") or p_data.get("patient_id") or p_idx)
            p_text = p_data.get("raw_text", "")

            source_file = p_data.get("source_file")
            source_stem = Path(source_file).stem if isinstance(source_file, str) and source_file.strip() else "input"

            patient_folder_name = self.safe_folder_name(f"{source_stem}_topic_{source_patient_id}")
            ...
            try:
                if PatientExtractor is None: raise ImportError("No se ha podido importar PatientExtractor")
                ...
                if hasattr(profile_data, "model_dump"): profile_data = profile_data.model_dump()
                
                profile_data.update({
                    "patient_id": internal_patient_id, "source_patient_id": source_patient_id,
                    "source": p_data.get("source"), "source_file": p_data.get("source_file"),
                    "input_format": p_data.get("input_format"), "raw_text": p_text
                })
                with open(patient_folder / "patient_profile.json", "w", encoding="utf-8") as f:
                    json.dump(profile_data, f, ensure_ascii=False, indent=2)
            except Exception as e:
                print(f"       Fallo en M2 PatientExtractor ({e}).")
                profile_data = {"patient_id": internal_patient_id, "patient_profile": {"condition": "Unknown"}}
            ...
            clinical_trials_client.search_from_plan(q_plan, output_path=raw_api)
            ...
            try:
                ext_obj = DirectedPatientExtractor(self.llm_client, registry_id=internal_patient_id).extract(
                    profile_data, registry, output_path=patient_folder / "extraction.json"
                )
                res_ext = ext_obj.model_dump() if hasattr(ext_obj, "model_dump") else ext_obj
            except Exception as e:
                print(f"       Aviso: Fallo en M10 ({e})")
                res_ext = {"patient_id": internal_patient_id, "attributes": []}
            ...
            for study in eval_data.get("unique_studies", []):
                ...
                for c in all_crit:
                    ...
                    if ((c.get("evaluation_status") == "unknown" or c.get("requires_missing_info")) and attribute_id):
                        attr_counter[attribute_id] += 1
                        if attribute_id not in attr_metadata:
                            attr_metadata[attribute_id] = {
                                "attribute_id": attribute_id,
                                "canonical_name": c.get("attribute", attribute_id),
                                "required_by": [{"trial_id": c.get("trial_id"), "criterion_text": c.get("raw_text")}],
                            }

            top_attrs = [attr_id for attr_id, count in attr_counter.most_common(12)]
            missing_attrs = [attr_metadata[attr_id] for attr_id in top_attrs]

            runner_q = InternalRunnerGen(cli=self.llm_client)
            preguntas_finales = []

            if missing_attrs:
                print(f"      Redactando {len(missing_attrs)} preguntas (Top Frecuentes) en paralelo...")
            ...
            print(f"       Se han redactado {len(preguntas_finales)} preguntas clínicas clave.")

            # [M13-M15] Question PDF + Ranking + Dossier
            print(f"    [M13-M15] Generando reportes finales...")
            q_manager = QuestionManager()
            unified_json = q_manager.unify_patient_questions(internal_patient_id, preguntas_finales, res_ext.get("attributes", []))
            ...
            ranking_output.update({"patient_id": source_patient_id, "topic_id": source_patient_id, "internal_patient_id": internal_patient_id})
            with open(ranking_path, "w", encoding="utf-8") as f: json.dump(ranking_output, f, ensure_ascii=False, indent=2)

            shutil.copy(str(ranking_path), str(m16_inputs_folder / f"topic_{source_patient_id}_ranking.json"))

            local_dossier_pdf = patient_folder / "Dossier_Ejecutivo.pdf"
            if self.generate_dossiers:
                self.generate_patient_dossier(ranking_output=ranking_output, output_path=local_dossier_pdf)

            shutil.copy(str(local_cuestionario_pdf), str(patient_output / f"1_Topic_{source_patient_id}_Cuestionario.pdf"))
            shutil.copy(str(ranking_path), str(patient_output / f"3_Topic_{source_patient_id}_Ranking.json"))
            if self.generate_dossiers and local_dossier_pdf.exists():
                shutil.copy(str(local_dossier_pdf), str(patient_output / f"2_Topic_{source_patient_id}_Dossier.pdf"))

        # Archivar XMLs
        for xml_path in xml_files:
            try: shutil.move(str(xml_path), str(input_archived / xml_path.name))
            except Exception as e: print(f"    Aviso: no se pudo archivar {xml_path.name}: {e}")

        # [M16] Prediction Exporter
        json_submission = None
        print("\n" + "─" * 80)
        print(f" [M16] Consolidando resultados globales del Batch #{batch_num}...")
        try:
            if PredictionExporter is None: raise ImportError("No se ha podido importar PredictionExporter")
            ...
            export_result = exporter.export_from_directory(
                input_dir=m16_inputs_folder, output_json_path=json_submission, output_trec_path=trec_submission,
            )
            print(f"    Exportación completada: {export_result.get('summary', {}).get('total_topics', 0)} pacientes.")
        except Exception as e: print(f"    Fallo en M16 ({e}).")
        ...
        if not qrels_files:
            print(f"    No se encontró archivo de Gold Standard. Saltando M17.")
            ...
            try:
                ...
                metrics = evaluator.evaluate(evaluator.load_predictions(str(json_submission)), evaluator.load_qrels(str(qrels_files[0])))
                ...
                plotter.plot_metrics(plotter.load_metrics(str(metrics_output)), str(batch_output_folder / "plots" / f"Batch{batch_num}_Metrics_Plot.png"))
                print(f"    Gráfico generado exitosamente en 'plots/'.")
            except Exception as e: print(f"    Fallo en M17 ({e}).")
